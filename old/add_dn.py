...
from utils import get_dn_number, get_inferenced_number
...
if __name__ == "__main__":
    ...
    if get_inferenced_number(db, nn_id) == get_dn_number(db):
        print(f"Already inferenced for \"{nn_name}\", overwrite? [Y/n]")
        input1 = input()
        ...
        if input1 == "n":
            ...
            with db.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                l = cur.execute("DELETE FROM dn_nn WHERE nn_id=%s", [ nn_id ])
            print("Deleted previous inference.")
